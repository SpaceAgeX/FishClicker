...
class UpgradeData():
	...
	upgradePriceMult = [2, 3, 4, 8]# Times Initial Price
	upgradesUnlock = 100
	upgradeOpen = False
	...
	def refresh(self):
		...
		for x in range(0, self.dimensions[1]):
			...
			for y in range(0, self.dimensions[0]):
				...
				button = UpgradeButton(((y*32)+self.pos[0]), (x*32+self.pos[1]), self.images[image], image, 1, random.randint(0,3), 4, text.ToolTip(self.images[image].get_rect, (150, 75), (47,77,47), "This is a Tool Tip"))
	...
	def draw(self, surface, prices):
		...
		for x in reversed(range(0, self.dimensions[1])):
			for y in reversed(range(0, self.dimensions[0])):
				if self.buttons[x][y].draw(surface, prices):
	...
	def draw(self, surface, prices):
		...
		if self.rect.collidepoint(pos):
			if pygame.mouse.get_pressed()[0] == 1 and self.clicked == False:
				self.clicked = True
				action = True
			self.tooltip.text = "Multiplier: "+str(UpgradeData.upgradeValue[self.frame]) +"\nPrice: " + str(int(UpgradeData.upgradePriceMult[self.frame]*prices[self.factory]))
